...
- **Неизвестные ключи** (без соответствующего argparse-аргумента) выводят предупреждение и игнорируются — так опечатки в конфиге не остаются незамеченными. Вложенные группы (dict-значения) этим путём не прокидываются — их применяют сами скрипты (например, `TF-IDF_LogReg` читает `config['tfidf']` напрямую).
