...
poetry run python ruintona/my_experiments/multimodal/late_fusion/Late_Fusion_CNN_BiLSTM_RuBERT.py --mode auto \
...
poetry run python ruintona/my_experiments/multimodal/late_fusion/Late_Fusion_Baseline_SVM_TF_IDF.py --mode auto

# Early fusion baseline
poetry run python ruintona/my_experiments/multimodal/early_fusion/Early_Fusion_Baseline.py --mode train

# Co-attention (needs large GPU memory, batch-size 4)
poetry run python ruintona/my_experiments/multimodal/co-attention/Co_Attention_Baseline.py --mode train
```
...
All 4 multimodal models are evaluated on `dusha_resd_test` (6616 samples) in the `multimodal_models_analise.ipynb` notebook, so they are directly comparable. The audio/text backbones of CNN-BiLSTM and HuBERT+RuBERT (CNN-BiLSTM, HuBERT, RuBERT) and the α=0.5 weights were trained/tuned on `combine_balanced`. HuBERT is a pretrained foundation model (description and source — [`model_analise/README.en.md`](../model_analise/README.en.md)). Corpus composition and building rules — [`CORPUS.en.md`](../../../CORPUS.en.md). Full reports — in `results/` (val/test, confusion matrices, per-epoch history).
