...
def test_audio_model_smoke(script_rel: str, audio_lmdb_pair: tuple[Path, Path]) -> None:
    ...
    script = PROJECT_ROOT / "ruintona" / "my_experiments" / script_rel
...
def test_random_forest_tune_smoke(audio_lmdb_pair: tuple[Path, Path]) -> None:
    ...
    train_path, test_path = audio_lmdb_pair
    script = PROJECT_ROOT / "ruintona" / "my_experiments" / "audio_models" / "baseline" / "random_forest.py"
    assert script.exists(), f"Script not found: {script}"
...
def test_cnn_bilstm_tune_smoke(audio_lmdb_pair: tuple[Path, Path]) -> None:
    ...
    train_path, test_path = audio_lmdb_pair
    script = PROJECT_ROOT / "ruintona" / "my_experiments" / "audio_models" / "CNN" / "CNN_BiLSTM.py"
    assert script.exists(), f"Script not found: {script}"
...
def test_stratified_subsample() -> None:
    ...
    from ruintona.my_experiments.audio_models.baseline.random_forest import _stratified_subsample
