...
Именно эти JSONL используются для построения LMDB и экспериментов (`ruintona/my_experiments/`).
...
>
> **Замечание про спектрограммы.** `lmdb_convert.py` **не записывает** mel-спектрограмму `x`. Для записей без `x` её можно досчитать из `waveform` скриптом `ruintona/data_processing/dataset/hug_dataset/add_missing_spectrograms.py` (нужно для `dusha_resd`). Существующие на диске LMDB `combine_balanced*` содержат `x` — они построены старой версией конвертера.
...
## Скрипты `ruintona/data_processing/dataset/hug_dataset/`
...
- [`ruintona/data_processing/README.md`](../../../../README.md) — пайплайн первичной обработки (Dawid-Skene, фичи)
- [`ruintona/data_processing/dataset/hug_dataset/`](../../../hug_dataset/) — скрипты для внешних HF-датасетов
