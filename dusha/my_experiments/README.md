# Эксперименты Dusha (`my_experiments/`)
...
Эксперименты работают с LMDB-базами из `data_processing/dataset/processed_dataset_090/aggregated_dataset/`. Пути хранятся в `dusha/my_experiments/data.json` (файл в `.gitignore`, т.к. содержит абсолютные пути):
...
Шаблон: `dusha/my_experiments/data.json.example`. Скопируйте и укажите свой путь:
...
JSON-конфиги гиперпараметров лежат в [`dusha/configs/`](../configs/README.md). Конфиг применяется к аргументам со значениями по умолчанию; явные CLI-флаги имеют приоритет.
...
- Модели сохраняются в `dusha/my_experiments/checkpoints/{text,audio,multimodal}/`.
...
Единая точка инференса — `dusha/my_experiments/inference.py` (модели: `audio`, `text`, `late-fusion`):
...
Демо-ноутбук: [`dusha/DEMO/`](../DEMO/README.md).
