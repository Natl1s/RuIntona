...
def _extract_label_to_index(model: AutoModelForAudioClassification | None) -> dict[str, int] | None:
...
def main():
    parser = argparse.ArgumentParser(
        description="Оценка whisper-large-v3 (audio classification) по тестовой выборке LMDB."
    )
    parser.add_argument(
        "--lmdb-path",
        type=Path,
    ...
    feature_extractor = AutoFeatureExtractor.from_pretrained(args.model_name, do_normalize=True)
    ...
    report_path = args.results_dir / f"whisper_large_v3_eval_{stem}.json"
