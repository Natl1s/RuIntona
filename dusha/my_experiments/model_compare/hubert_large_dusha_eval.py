...
DEFAULT_MODEL_ID = "xbgoose/hubert-speech-emotion-recognition-russian-dusha-finetuned"
...
def _prepare_model_inputs(
    ...
    arrays = [
        _prepare_audio_array(sample["waveform"], sample["sample_rate"], target_sr, max_length)
...
def _run_model_batch(
    ...
    inputs = _prepare_model_inputs(feature_extractor, samples, max_duration, device)
    ...
    try:
        ...
        with env.begin() as txn:
            for idx in tqdm(range(total), desc="Чтение LMDB", unit="sample"):
                ...
                yield waveform, sample_rate, label
    finally:
        env.close()
...
def main():
    parser = argparse.ArgumentParser(
        description="Оценка HuBERT large по тестовой выборке LMDB."
    )
    parser.add_argument(
        "--lmdb-path",
        type=Path,
    ...
    parser.add_argument("--model-name", type=str, default=DEFAULT_MODEL_ID)
    parser.add_argument(
    ...
    feature_extractor = Wav2Vec2FeatureExtractor.from_pretrained(args.feature_extractor_name)
    model = HubertForSequenceClassification.from_pretrained(args.model_name)
    model.to(device)
    model.eval()
    ...
    for waveform, sample_rate, label in _iter_lmdb_samples(args.lmdb_path, skipped):
        samples.append({"waveform": waveform, "sample_rate": sample_rate})
        labels.append(label)
        if len(samples) >= args.batch_size:
            outputs = _run_model_batch(
                model,
                feature_extractor,
                samples,
                device,
    ...
    report_path = args.results_dir / f"hubert_large_dusha_eval_{stem}.json"
