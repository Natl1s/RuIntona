...
from dusha.my_experiments.utils.config_utils import get_dataset_path, TARGET_NAMES, get_dataset_name, models_dir_for, load_experiment_config, apply_config_to_args, add_config_arg, add_data_path_args, resolve_data_paths
from dusha.my_experiments.utils.model_io import save_sklearn_model, load_sklearn_model, sklearn_model_exists
from dusha.my_experiments.utils.lmdb_utils import load_feature_vectors_from_lmdb, to_fixed_vector
from dusha.my_experiments.utils.metrics import weighted_accuracy
...
class _EmotionStringPredictor:
    """Обёртка над sklearn-бустингом, обученным на целочисленных индексах 0..3.

    XGBoost/LightGBM требуют числовые метки при ``fit``, а весь код оценки
    (``evaluate_sklearn_classifier``, ``_align_proba_to_emotions``) работает со
    строковыми эмоциями. Обёртка транслирует ``predict()`` в строки, остальные
    атрибуты (``predict_proba``, ``feature_importances_``, ``get_params`` и т.д.)
    проксируются на исходную модель.
    """
    ...
    def predict(self, X):
        return _to_emotion_labels(self.model.predict(X))
...
def _as_emotion_string_predictor(model):
    """Оборачивает модель, только если её predict() возвращает индексы, а не строки."""
    if isinstance(model, _EmotionStringPredictor):
        return model
    sample_classes = np.asarray(getattr(model, "classes_", []))
    if sample_classes.size:
        first = sample_classes.ravel()[0]
        if str(first).strip().lstrip("-").isdigit():
            return _EmotionStringPredictor(model)
    return model
...
def _resolve_feature_level(name):
    import opensmile
...
def _init_opensmile_extractor(feature_set=None, feature_level=None):
    try:
        import opensmile
...
def load_opensmile_features_from_manifest(
    ...
    smile = _init_opensmile_extractor(feature_set=feature_set, feature_level=feature_level)
    features = []
    labels = []
    feature_names = None
    focus_stats_rows = []
...
def _build_classifier(
    ...
    errors = []
    xgb_extra = xgboost_params or {}
    lgb_extra = lightgbm_params or {}
    ...
    def _try_xgboost():
        ...
        return XGBClassifier(
            ...
            max_depth=max_depth,
            subsample=xgb_extra.get("subsample", 0.9),
            colsample_bytree=xgb_extra.get("colsample_bytree", 0.9),
            reg_lambda=xgb_extra.get("reg_lambda", 1.0),
            reg_alpha=xgb_extra.get("reg_alpha", 0.0),
            eval_metric=xgb_extra.get("eval_metric", "mlogloss"),
            tree_method=xgb_extra.get("tree_method", "hist"),
    ...
    def _try_lightgbm():
        ...
        return LGBMClassifier(
            ...
            learning_rate=learning_rate,
            max_depth=max_depth,
            subsample=lgb_extra.get("subsample", 0.9),
            colsample_bytree=lgb_extra.get("colsample_bytree", 0.9),
            reg_lambda=lgb_extra.get("reg_lambda", 1.0),
            reg_alpha=lgb_extra.get("reg_alpha", 0.0),
            random_state=random_state,
            n_jobs=-1,
        ), "lightgbm"
...
def evaluate_model(model, X_train, y_train, X_test, y_test, feature_names, focus_stats_train):
    ...
    resolved_model_name = getattr(model, "model", model).__class__.__name__

    y_train_str = _to_emotion_labels(y_train)
    y_test_str = _to_emotion_labels(y_test)

    def _print_params(m):
        print_model_parameters(m, resolved_model_name, feature_names)

    metrics = evaluate_sklearn_classifier(
        model, X_train, y_train_str, X_test, y_test_str,
        transform_fn=None,
    ...
    y_test_idx = _to_emotion_indices(y_test_str)

    extra_metrics = {
        "test_balanced_accuracy": float(balanced_accuracy_score(y_test_str, test_pred)),
        "test_f1_macro": float(f1_score(y_test_str, test_pred, labels=EMOTIONS, average="macro", zero_division=0)),
        "test_f1_weighted": float(f1_score(y_test_str, test_pred, labels=EMOTIONS, average="weighted", zero_division=0)),
        "test_precision_macro": float(precision_score(y_test_str, test_pred, labels=EMOTIONS, average="macro", zero_division=0)),
        "test_recall_macro": float(recall_score(y_test_str, test_pred, labels=EMOTIONS, average="macro", zero_division=0)),
        "test_UAR": float(recall_score(y_test_str, test_pred, labels=EMOTIONS, average="macro", zero_division=0)),
        "test_WA": float(weighted_accuracy(y_test_str, test_pred, labels=EMOTIONS)),
        "test_mcc": float(matthews_corrcoef(y_test_str, test_pred)),
        "test_cohen_kappa_qwk": float(cohen_kappa_score(y_test_str, test_pred, labels=EMOTIONS, weights="quadratic")),
    ...
    try:
        extra_metrics["test_roc_auc_ovr_macro"] = float(
            roc_auc_score(y_test_idx, test_proba, multi_class="ovr", average="macro")
        )
    except ValueError:
        extra_metrics["test_roc_auc_ovr_macro"] = float("nan")

    if hasattr(model, "feature_importances_"):
        top_idx = np.argsort(model.feature_importances_)[-20:][::-1]
        extra_metrics["top_20_feature_importances"] = [
...
def train_opensmile_boosting(
    ...
    X_train, y_train, feature_names, focus_stats_train, n_train_skipped = load_opensmile_features_from_manifest(
        train_manifest, dataset_root=get_dataset_path(), cache_path=train_cache,
    ...
    if save:
        training_params = {
            ...
            "max_depth": max_depth,
            "random_state": random_state,
            "feature_set": feature_set or "eGeMAPSv02",
            "feature_level": feature_level or "Functionals",
            "xgboost_params": xgboost_params or {},
...
if __name__ == "__main__":
    ...
    # Вложенные группы конфига (opensmile/xgboost/lightgbm) скрипт применяет сам:
    # apply_config_to_args пропускает dict-значения. Параметры, у которых есть
    # CLI-флаг (n_estimators/learning_rate/max_depth/random_state), из вложенных
    # групп не берём — они задаются плоскими ключами/флагами.
    _NESTED_FLAT_KEYS = {"n_estimators", "learning_rate", "max_depth", "random_state"}
    opensmile_cfg = (experiment_config or {}).get("opensmile", {}) or {}
    xgboost_extra = {
        k: v for k, v in ((experiment_config or {}).get("xgboost", {}) or {}).items()
        if k not in _NESTED_FLAT_KEYS
    }
    lightgbm_extra = {
        k: v for k, v in ((experiment_config or {}).get("lightgbm", {}) or {}).items()
        if k not in _NESTED_FLAT_KEYS
    }
    ...
    common_kwargs = {
        ...
        "xgboost_params": xgboost_extra or None,
        "lightgbm_params": lightgbm_extra or None,
        "feature_set": opensmile_cfg.get("feature_set"),
        "feature_level": opensmile_cfg.get("feature_level"),
