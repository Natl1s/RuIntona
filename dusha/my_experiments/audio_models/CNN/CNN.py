...
from dusha.my_experiments.utils.config_utils import TARGET_NAMES, get_dataset_name, models_dir_for, load_experiment_config, add_data_path_args, resolve_data_paths
from dusha.my_experiments.utils.model_io import save_pytorch_model, load_pytorch_model, pytorch_model_exists
from dusha.my_experiments.utils.torch_utils import set_seed, resolve_device, evaluate_split, EarlyStopping
...
def train_cnn(
    ...
    for epoch in range(1, epochs + 1):
        ...
        val_metrics, _, _, _, _ = evaluate_split(model, val_loader, criterion, device, desc="Val")
    ...
    final_test_metrics, y_true, y_pred, _, _ = evaluate_split(model, test_loader, criterion, device, desc="Test")
...
def main():
    ...
    for key in ["epochs", "batch_size", "lr", "weight_decay", "seed", "device", "classifier_dropout", "val_size", "early_stopping_patience"]:
        attr = key.replace("-", "_")
    ...
    if args.mode == "smoke":
        ...
        train_cnn(
            ...
            val_size=cfg["val_size"], early_stopping_patience=cfg["early_stopping_patience"],
        )
