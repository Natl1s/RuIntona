...
from dusha.my_experiments.utils.config_utils import DATASET_PATH, EMO2LABEL, LABEL2EMO, TARGET_NAMES, models_dir_for, load_experiment_config, apply_config_to_args, add_config_arg, add_data_path_args, resolve_data_paths
from dusha.my_experiments.utils.model_io import save_pytorch_model, load_pytorch_model, pytorch_model_exists, load_torch_with_weights
from dusha.my_experiments.utils.torch_utils import set_seed, resolve_device, compute_classification_metrics, EarlyStopping, _eval_collect
...
def evaluate_split(model, loader, criterion, device, use_amp: bool):
    def forward_fn(m, batch):
        ...
        return torch.clamp(m(waves, attention_mask), -20, 20)
...
def train_wav2vec(
    ...
    loss_ma_window: int,
    use_amp: bool,
    resume_from_warm_start: bool = True,
):
    set_seed(seed)
    ...
    # is_train=True for both train and val crops (val_ds accesses full_train_ds which has is_train=True)

    train_loader = _build_loader(train_ds, batch_size=batch_size, shuffle=True, use_cuda=use_cuda, num_workers=num_workers)
    val_loader = _build_loader(val_ds, batch_size=batch_size, shuffle=False, use_cuda=use_cuda, num_workers=num_workers)
    ...
    if resume_from_warm_start:
        ...
        global_step = 0
        start_epoch = 1
        print("Свежий старт (без warm-start checkpoint): обучение с эпохи 1, только head.")
    ...
    if start_epoch > 1:
        ...
        print("Старт обучения с эпохи 1 (head-only, энкодер заморожен).")
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        if do_eval:
            val_metrics, _, _, _, _ = evaluate_split(model, val_loader, criterion, device, use_amp=use_amp)
