...
from dusha.my_experiments.audio_models.CNN.CNN_BiLSTM import EmotionCNNBiLSTM
from dusha.my_experiments.text_models.transformers.RuBERT import EmotionClassifier
...
from dusha.my_experiments.utils.lmdb_utils import (
    get_lmdb_length,
    open_lmdb_readonly,
    parse_label_to_index,
    safe_pickle_loads,
)
from dusha.my_experiments.utils.model_io import (
    load_pytorch_model,
    load_torch_with_weights,
    save_pytorch_model,
)
from dusha.my_experiments.utils.text_utils import extract_text
from dusha.my_experiments.utils.torch_utils import resolve_device, set_seed
...
class EarlyFusionModel(nn.Module):
    ...
    def forward_features(
        ...
        """Обучаемая часть: attention-pooling + проекции + MLP поверх фич."""
    ...
    def forward(
        ...
        audio_seq, seq_lengths, text_vec = self.extract_features(
            audio_x, audio_lengths, input_ids, attention_mask
        )
        return self.forward_features(audio_seq, seq_lengths, text_vec)
...
def evaluate_split(model, loader, criterion, device: torch.device, use_cuda: bool, desc: str, forward_fn=None):
    model.eval()
    running_loss = 0.0
    all_true = []
    all_pred = []
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc=desc, leave=False):
            *data, labels = batch
            data = [t.to(device, non_blocking=use_cuda) for t in data]
...
def save_model(
    ...
    """Сохраняет модель в едином формате checkpoint (model_io.save_pytorch_model)."""
    tokenizer_dir = MODELS_DIR / f"{model_name}_{dataset_name}_tokenizer"
    tokenizer.save_pretrained(tokenizer_dir)

    extra_artifacts = {"tokenizer_dir.txt": str(tokenizer_dir)}
    save_pytorch_model(
        model.state_dict(),
...
def load_model(
...
) -> tuple[EarlyFusionModel, AutoTokenizer]:
    """Восстанавливает EarlyFusionModel из единого checkpoint (model_io.load_pytorch_model)."""
    checkpoint = load_pytorch_model(
        dataset_name,
        models_dir=MODELS_DIR,
        model_name=model_name,
    ...
    if not model_params:
        raise ValueError(
            "Checkpoint не содержит 'model_params'; реконструкция EarlyFusionModel невозможна."
        )

    audio_model = EmotionCNNBiLSTM(
        n_classes=model_params.get("n_classes", len(TARGET_NAMES)),
        lstm_hidden_size=model_params["audio_lstm_hidden_size"],
        lstm_layers=model_params["audio_lstm_layers"],
        lstm_dropout=model_params["audio_lstm_dropout"],
        bidirectional=bool(model_params["audio_bidirectional"]),
    )
    text_model = EmotionClassifier(
        model_name=model_params["text_backbone_name"],
        num_classes=model_params.get("text_num_classes", len(TARGET_NAMES)),
        dropout=model_params.get("text_dropout", 0.0),
        classifier_hidden_size=model_params.get("text_classifier_hidden_size"),
    )
    model = EarlyFusionModel(
        audio_encoder=FrozenAudioEncoder(audio_model),
    ...
    if tokenizer_dir.exists():
        ...
        tokenizer = AutoTokenizer.from_pretrained(model_params["text_backbone_name"])
...
def load_audio_encoder(
    ...
    state_dict = load_torch_with_weights(audio_model_path, map_location=device)
...
def load_text_encoder(
    ...
    tokenizer_dir = Path(str(text_model_path).split("_model")[0] + "_tokenizer")
    if tokenizer_dir.exists():
        tokenizer = AutoTokenizer.from_pretrained(tokenizer_dir)
    else:
...
class CachedFeatureDataset(Dataset):
    """Читает предвычисленные фичи замороженных энкодеров из кэш-LMDB."""

    def __init__(self, cache_path: Path, labels_path: Path):
        self.cache_path = Path(cache_path)
        self.env = open_lmdb_readonly(self.cache_path)
        self.length = get_lmdb_length(self.env)
        self.labels = np.load(labels_path).astype(np.int64)
        if self.length <= 0 or self.labels.shape[0] != self.length:
            raise ValueError(f"Несогласованный кэш фич: {self.cache_path}")
    ...
    def __getitem__(self, idx: int):
        ...
        if raw is None:
            raise KeyError(f"В кэше фич отсутствует ключ {idx}")
        payload = safe_pickle_loads(raw)
        audio_seq = torch.from_numpy(np.asarray(payload["audio_seq"], dtype=np.float32))
        length = int(payload["length"])
        text_vec = torch.from_numpy(np.asarray(payload["text_vec"], dtype=np.float32))
        label = int(payload["label"])
        return audio_seq, length, text_vec, label


def cached_collate(batch):
    """Колл-функция для CachedFeatureDataset: паддинг audio_seq по батч-max T."""
    audio_seqs, lengths, text_vecs, labels = zip(*batch)
    lengths_t = torch.tensor(lengths, dtype=torch.long)
    max_t = int(lengths_t.max().item())
    padded = []
    for seq in audio_seqs:
        delta = max_t - seq.shape[0]
        if delta > 0:
            seq = nn.functional.pad(seq, pad=(0, 0, 0, delta))
        padded.append(seq)
    audio_batch = torch.stack(padded, dim=0)
    text_batch = torch.stack(list(text_vecs), dim=0)
    label_batch = torch.tensor(labels, dtype=torch.long)
    return audio_batch, lengths_t, text_batch, label_batch


def _cache_meta_sources(args) -> dict:
    """Параметры, определяющие валидность кэша (данные + модели)."""
    return {
        "train_lmdb": str(args.train_lmdb),
        "test_lmdb": str(args.test_lmdb),
        "train_lmdb_size": int(args.train_lmdb.stat().st_size),
        "test_lmdb_size": int(args.test_lmdb.stat().st_size),
        "train_lmdb_mtime": float(args.train_lmdb.stat().st_mtime),
        "test_lmdb_mtime": float(args.test_lmdb.stat().st_mtime),
        "audio_model_path": str(args.audio_model_path),
        "text_model_path": str(args.text_model_path),
        "audio_model_mtime": float(args.audio_model_path.stat().st_mtime),
        "text_model_mtime": float(args.text_model_path.stat().st_mtime),
        "audio_lstm_hidden_size": int(args.audio_lstm_hidden_size),
        "audio_lstm_layers": int(args.audio_lstm_layers),
        "audio_lstm_dropout": float(args.audio_lstm_dropout),
        "audio_unidirectional": bool(args.audio_unidirectional),
...
def _feature_cache_paths(cache_dir: Path, args) -> dict[str, tuple[Path, Path]] | None:
    """Возвращает пути к кэшу, если он существует и актуален, иначе None."""
    meta_path = cache_dir / CACHE_META
    required = {
        "train": (cache_dir / "train.lmdb", cache_dir / "train_labels.npy"),
        "test": (cache_dir / "test.lmdb", cache_dir / "test_labels.npy"),
    }
    all_files_exist = meta_path.exists() and all(
        p.exists() for pair in required.values() for p in pair
    )
    ...
    try:
        ...
        return None
    if meta.get("sources") != _cache_meta_sources(args):
        print("Кэш фич устарел (изменились данные или модели) — пересоздаю.")
        return None
    return required
...
def _write_feature_cache(
    ...
    """Однократный проход: извлекает фичи замороженных энкодеров и пишет кэш."""
    cache_dir.mkdir(parents=True, exist_ok=True)
    model.eval()
    paths = {}
    for split, loader in split_loaders.items():
        cache_path = cache_dir / f"{split}.lmdb"
        labels_path = cache_dir / f"{split}_labels.npy"
        if cache_path.exists() and labels_path.exists():
            paths[split] = (cache_path, labels_path)
            continue
        map_size = 18 * 1024**3 if split == "train" else 2 * 1024**3
        env = lmdb.open(str(cache_path), subdir=False, map_size=map_size)
        labels = []
        written = 0
        with env.begin(write=True) as txn:
            for audio_x, audio_lengths, input_ids, attention_mask, batch_labels in tqdm(
                loader,
                desc=f"Извлечение фич ({split})",
                leave=False,
            ):
                ...
                audio_seq, seq_lengths, text_vec = model.extract_features(
                    audio_x, audio_lengths, input_ids, attention_mask
                )
                audio_seq = audio_seq.detach().cpu().numpy()
                seq_lengths = seq_lengths.detach().cpu().numpy()
                text_vec = text_vec.detach().cpu().numpy()

                for i in range(audio_seq.shape[0]):
                    payload = {
                        "audio_seq": audio_seq[i, : int(seq_lengths[i])],
                        "length": int(seq_lengths[i]),
                        "text_vec": text_vec[i],
                        "label": int(batch_labels[i]),
                    }
                    txn.put(str(written).encode("utf-8"), pickle.dumps(payload, protocol=4))
        ...
        env.close()
        paths[split] = (cache_path, labels_path)
        print(f"Кэш фич {split}: {written} примеров -> {cache_path}")

    meta = {"sources": sources}
    (cache_dir / CACHE_META).write_text(json.dumps(meta, ensure_ascii=False, indent=2), encoding="utf-8")
    return paths
...
def main():
    ...
    parser.add_argument(
        "--cache-dir",
        type=Path,
        default=CHECKPOINTS_DIR / "multimodal" / "early_fusion_features",
        help="Каталог кэша фич замороженных энкодеров (пересоздаётся при изменении данных/моделей).",
    )
    ...
    use_cache = args.mode != "smoke"

    if use_cache:
        cache_paths = _feature_cache_paths(args.cache_dir, args)
        if cache_paths is None:
            print(f"Извлечение фич замороженных энкодеров -> {args.cache_dir}")
            raw_train_ds = FusionLmdbDataset(args.train_lmdb)
            raw_test_ds = FusionLmdbDataset(args.test_lmdb)
            extract_collate = build_collate_fn(tokenizer=tokenizer, max_len=max_len)
            extract_bsz = max(64, args.batch_size)
            raw_train_loader = DataLoader(
                raw_train_ds,
                batch_size=extract_bsz,
                shuffle=False,
                num_workers=0,
                pin_memory=use_cuda,
            ...
            cache_paths = _write_feature_cache(
                cache_dir=args.cache_dir,
                split_loaders={"train": raw_train_loader, "test": raw_test_loader},
                model=model,
                device=device,
                use_cuda=use_cuda,
                sources=_cache_meta_sources(args),
            )
        else:
            print(f"Использую существующий кэш фич: {args.cache_dir}")
        train_ds = CachedFeatureDataset(*cache_paths["train"])
        test_ds = CachedFeatureDataset(*cache_paths["test"])
        collate_fn = cached_collate

        def forward_fn(batch_data):
            return model.forward_features(*batch_data)
    ...
    model_params = {
        ...
        "text_backbone_name": str(text_model_params["backbone_name"]),
        "text_num_classes": int(text_model_params["n_classes"]),
        "text_dropout": float(text_model_params["dropout"]),
        "text_classifier_hidden_size": (
            int(text_model_params["classifier_hidden_size"])
            if text_model_params.get("classifier_hidden_size") is not None
            else None
        ),
    }
