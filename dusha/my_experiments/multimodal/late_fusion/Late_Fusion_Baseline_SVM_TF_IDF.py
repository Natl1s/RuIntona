...
from dusha.my_experiments.utils.lmdb_utils import get_lmdb_length, open_lmdb_readonly, parse_label_to_index, to_fixed_vector, safe_pickle_loads
from dusha.my_experiments.utils.config_utils import (
    MY_EXPERIMENTS_DIR, TARGET_NAMES, CHECKPOINTS_DIR,
    find_pretrained_model, resolve_model_path, resolve_sklearn_artifact_path,
    load_experiment_config, apply_config_to_args, add_config_arg, add_data_path_args, resolve_data_paths,
)
from dusha.my_experiments.utils.model_io import load_pytorch_model, load_sklearn_model
from dusha.my_experiments.utils.torch_utils import set_seed
...
DEFAULT_AUDIO_MODEL_PATH = (
    find_pretrained_model("audio", "svm", "combine_balanced_train")
    or CHECKPOINTS_DIR / "audio" / "svm_combine_balanced_train_model.pkl"
)
DEFAULT_AUDIO_SCALER_PATH = resolve_sklearn_artifact_path(
    "audio", "svm", "combine_balanced_train", "scaler"
)

DEFAULT_TEXT_MODEL_PATH = (
    find_pretrained_model("text", "TF-IDF_LogReg", "combine_balanced_train")
    or CHECKPOINTS_DIR / "text" / "TF-IDF_LogReg_combine_balanced_train_model.pkl"
)
DEFAULT_TEXT_VECTORIZER_PATH = resolve_sklearn_artifact_path(
    "text", "TF-IDF_LogReg", "combine_balanced_train", "vectorizer"
)
...
class FusionLmdbDataset:
        ...
        with self.env.begin() as txn:
            for idx in tqdm(
                ...
                if "x" not in payload:
                    continue
                try:
                    to_fixed_vector(np.asarray(payload["x"], dtype=np.float32))
    ...
    def __getitem__(self, item_idx: int):
        ...
        audio_vec = to_fixed_vector(np.asarray(payload["x"], dtype=np.float32))
