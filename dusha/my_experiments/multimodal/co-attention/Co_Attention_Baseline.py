...
from dusha.my_experiments.utils.lmdb_utils import get_lmdb_length, open_lmdb_readonly, parse_label_to_index, safe_pickle_loads
...
from dusha.my_experiments.utils.torch_utils import set_seed, resolve_device
...
DEFAULT_AUDIO_WARM_START_PATH = (
    find_pretrained_model("audio", "wav2vec2_xlsr300m_self_attention", "combine_balanced_train_small")
    or CHECKPOINTS_DIR / "audio" / "wav2vec2_xlsr300m_self_attention_combine_balanced_train_small_model.pt"
)
...
def _load_text_backbone(
    ...
    checkpoint = load_torch_with_weights(text_model_path, map_location="cpu")
...
def _load_audio_warm_start(audio_encoder: AudioSequenceEncoder, warm_start_path: Path) -> None:
    checkpoint = load_torch_with_weights(warm_start_path, map_location="cpu")
...
def save_model(
    ...
    save_pytorch_model(
        ...
        model_class="CrossModalCoAttentionModel",
...
def load_model(
...
) -> tuple[CrossModalCoAttentionModel, AutoTokenizer]:
    """Восстанавливает CrossModalCoAttentionModel из единого checkpoint."""
    ...
    if not model_params:
        raise ValueError(
            "Checkpoint не содержит 'model_params'; реконструкция CrossModalCoAttentionModel невозможна."
        )

    audio_encoder = AudioSequenceEncoder(
        pretrained_name=model_params["audio_pretrained_name"],
        gradient_checkpointing=False,
    )
    text_backbone = AutoModel.from_pretrained(model_params["text_backbone_name"])
    ...
    model = CrossModalCoAttentionModel(
        ...
        n_blocks=model_params["num_coattn_blocks"],
        dropout=model_params["dropout"],
        ffn_mult=model_params["ffn_mult"],
...
def _load_resume_checkpoint(checkpoint_path: Path) -> dict:
    checkpoint = load_torch_with_weights(checkpoint_path, map_location="cpu")
    if not isinstance(checkpoint, dict):
...
def main():
    ...
    model_params = {
        "audio_pretrained_name": str(args.audio_pretrained_name),
        "audio_gradient_checkpointing": bool(args.gradient_checkpointing),
        "d_model": int(args.d_model),
        "num_heads": int(args.num_heads),
        "num_coattn_blocks": int(args.num_coattn_blocks),
        "dropout": float(args.dropout),
    ...
    training_params = _args_payload(args)
