"""
Инференс обученных моделей Dusha: audio / text / late-fusion.

Единая точка для предсказания эмоции по аудиофайлу и/или тексту.

Примеры:
    # Мультимодально (аудио + текст)
    python dusha/my_experiments/inference.py --model late-fusion \
        --audio dusha/DEMO/data/001ce26c07c20eaa0d666b824c6c6924.wav \
        --text "шестьдесят тысяч тенге сколько будет стоить"

    # Только аудио
    python dusha/my_experiments/inference.py --model audio --audio sample.wav

    # Только текст
    python dusha/my_experiments/inference.py --model text --text "привет!"
"""
...
from dusha.my_experiments.multimodal.late_fusion.Late_Fusion_CNN_BiLSTM_RuBERT import (
    load_audio_model as _load_audio_model,
)
from dusha.my_experiments.text_models.transformers.RuBERT import EmotionClassifier
from dusha.my_experiments.utils.config_utils import CHECKPOINTS_DIR, TARGET_NAMES, find_pretrained_model
from dusha.my_experiments.utils.model_io import load_torch_with_weights
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Инференс моделей Dusha (audio / text / late-fusion)."
    )
    parser.add_argument(
        "--model",
        choices=list(MODELS),
