...
These JSONL files are used to build the LMDBs and run the experiments (`dusha/my_experiments/`).
...
>
> **About spectrograms.** `lmdb_convert.py` does **not** write the mel-spectrogram `x`. For records without `x` compute it from `waveform` with `dusha/data_processing/dataset/hug_dataset/add_missing_spectrograms.py` (required for `dusha_resd`). The existing `combine_balanced*` LMDBs on disk do contain `x` — they were built by an older converter version.
...
## Scripts in `dusha/data_processing/dataset/hug_dataset/`
...
- [`dusha/data_processing/README.md`](../../../../README.md) — raw processing pipeline (Dawid-Skene, features)
- [`dusha/data_processing/dataset/hug_dataset/`](../../../hug_dataset/) — scripts for external HF datasets
