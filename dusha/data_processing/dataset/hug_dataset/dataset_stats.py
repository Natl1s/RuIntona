...
_data_config_path = Path(__file__).resolve().parents[3] / "experiments" / "configs" / "data.config"
_data_config_ns: dict[str, Any] = {}
exec(_data_config_path.read_text(encoding="utf-8"), _data_config_ns)
DATASET_PATH: Path = _data_config_ns["base_path"]

DEFAULT_TRAIN_PATH = Path(
    DATASET_PATH / "hug_dataset" / "data" / "raw_train-00000-of-00001-1f5fe73d1293189c.jsonl"
)
DEFAULT_TEST_PATH = Path(
    DATASET_PATH / "hug_dataset" / "data" / "raw_test-00000-of-00001-a2b788d59856c4ae.jsonl"
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--train", type=Path, default=DEFAULT_TRAIN_PATH, help="Путь к train JSONL")
    parser.add_argument("--test", type=Path, default=DEFAULT_TEST_PATH, help="Путь к test JSONL")
    parser.add_argument(
        "--label-priority",
        nargs="+",
