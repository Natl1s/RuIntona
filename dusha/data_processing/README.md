# Обработка данных Dusha (`dusha/data_processing/`)
...
raw (crowd.tar / podcast.tar) → processing.py → processed_dataset_0XX/
                                                     ...
                                                     ├── train/*.jsonl
                                                     └── test/*.jsonl
                                                        → make_data_scripts/ → LMDB → dusha/my_experiments/
```
...
# либо вручную из dusha/data_processing/requirements.txt:
...
```bash
poetry run python dusha/data_processing/processing.py -dataset_path DATASET_PATH
```
