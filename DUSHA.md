...
- В этом репозитории используется порог уверенности **0.9** → результат обработки лежит в `processed_dataset_090` (см. [`dusha/data_processing/README.md`](dusha/data_processing/README.md)).
...
- **Адаптированный материал** из оригинального проекта [Salute Developers — Golos/Dusha](https://github.com/salute-developers/golos):
  - пайплайн `dusha/data_processing/` (обработка сырых данных, извлечение признаков, агрегация меток Dawid-Skene),
  - структуру датасета и производные артефакты датасета (признаки, манифесты, базы LMDB).
- **Оригинальную работу** автора этого репозитория: всё остальное в `dusha/my_experiments/`, `dusha/DEMO/`, `dusha/configs/` — лицензировано отдельно (см. [LICENSE](./LICENSE)).
